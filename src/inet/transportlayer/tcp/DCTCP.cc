...
#include "inet/transportlayer/tcp/DCTCP.h"
...
//  the number of ACKs that is necessary for calculating the proportion of ECN-marked ACK (F)
void DCTCP::calcuF()
{
    ecn_f = double(sum_ecn)/double(sum_ack);
    EV<<" The ecn_f = "<<ecn_f<<endl;
}

void DCTCP::updateandcutwnd()
{
    ecn_g = 0.0625;
    //update alpha based on DCTCP
    ecn_a = (1-ecn_g)*ecn_a + ecn_g*ecn_f;
    //cut window based on DCTCP
    state->snd_cwnd = state->snd_cwnd*(1-ecn_a/2);
    EV_INFO << "cwnd has been cut by the ECN algorithm with the percentage of: "<< ecn_a<<", and the updated cwnd is: "<<state->snd_cwnd<<endl;
}

void DCTCP::receivedDataAck(uint32 firstSeqAcked,bool ecn)
{
    TcpTahoeRenoFamily::receivedDataAck(firstSeqAcked,ecn);
    ...
    EV<< " receiveDataAck(), receive an ecn-marked ack, ecn = "<<ecn<<", sum_ecn = "<<sum_ecn<<", sum_ack = "<<sum_ack<<endl;
    // to perform ECN_related window-cut every RTT.
    if(simTime() - last_control_time >= state->srtt)
    {
        calcuF();
        updateandcutwnd();
...
// means packet drop, congestion reaction begin.
