...
int BehaviorAggregateClassifier::classifyPacket(Packet *packet)
{
    ...
    EV <<"1111 the dscp value = "<<dscp<<endl;
    if (dscp >= 0) {
        auto it = dscpToGateIndexMap.find(dscp);
        if (it != dscpToGateIndexMap.end())
        {
            EV <<"1111 come into find dscp"<<endl;
            return it->second;
        }
    }
    return -1;
...
#ifdef WITH_IPv4
    if (protocol == &Protocol::ipv4||protocol == &Protocol::tcp) {
        EV <<"1111 come into getDscpFromPacket"<<endl;
