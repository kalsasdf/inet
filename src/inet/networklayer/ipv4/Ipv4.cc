...
void Ipv4::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        hasEcmp = par("useEcmp");
        multipath = par("enableMultiPath");
        reverse = par("reverse");
        use_midware = par("midWare");
        ...
        Ipv4Address ipv4addr = rt->getRouterId();
        WATCH(ipv4addr);
        WATCH(numMulticast);
...
void Ipv4::handleMessage(cMessage *msg)
{
    ...
    else if (string(msg->getFullName()).find("Suspend") != string::npos)
    {
        ProcessSuspend(check_and_cast<Packet*>(msg));
    }
    else if (string(msg->getFullName()).find("IfRegisterSignal") != string::npos)
    {
        RegistMidwareInterface(check_and_cast<Packet*>(msg));
    }
    else if (string(msg->getFullName()).find("Pause") != string::npos)
    {
        ProcessPause(check_and_cast<Packet*>(msg));
    }
    else
    {
...
void Ipv4::preroutingFinish(Packet *packet)
{
    ...
    if (fromIE->isLoopback()) {
        ...
        if (rt->isLocalAddress(destAddr) || (fromIE->ipv4Data()->getIPAddress().isUnspecified() && string(packet->getFullName()).find("Data") == string::npos)) {
            reassembleAndDeliver(packet);
        }
...
void Ipv4::handlePacketFromHL(Packet *packet)
{
    ...
    EV <<"1111 protocol = "<<protocol<<endl;
...
void Ipv4::routeUnicastPacket(Packet *packet)
{
    ...
    // modified by shan huang, getting the vpid value of packet.
    //int vpid = packet->getSchedulingPriority();
    uint32_t vpid = packet->getPathID();
    const auto& ipv4Header = packet->peekAtFront<Ipv4Header>();
    Ipv4Address destAddr = ipv4Header->getDestAddress();
    Ipv4Address srcAddr = ipv4Header->getSrcAddress();

    EV <<"routeUnicastPacket(), entering routeUnicastPacket, vpid = "<<vpid<<endl;
    ...
    if (destIE) {
        ...
        if(reverse)
        {
            if(hasEcmp)
            {
                if (multipath)
                {
                    ...
                    re = rt->findreverseRoute(srcAddr,destAddr,vpid,true);
                    if(re == nullptr)
                    {
                        EV<<"routeUnicastPacket() by path id, the IF_in id = "<<IF_in<<endl;
                        re = rt->findecmpRoute(IF_in,srcAddr,destAddr,vpid,true);
                    }
                }
                else
                {
                    IF_in = getSourceInterface(packet)->getInterfaceId();
                    re = rt->findreverseRoute(srcAddr,destAddr,vpid,false);
                    if(re == nullptr)
                    {
                        EV<<"routeUnicastPacket(), the IF_inid = "<<IF_in<<endl;
                        re = rt->findecmpRoute(IF_in,srcAddr,destAddr,vpid,false);
                    }
                }
            }
            else{
        ...
        if (re) {
                ...
                packet->addTagIfAbsent<InterfaceReq>()->setInterfaceId(re->getInterface()->getInterfaceId());
                packet->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(re->getGateway());
        }
    }
...
// added by angrydudu. 12/05/2019
void Ipv4::ProcessSuspend(Packet *packet)
{
    const InterfaceEntry *fromIE = getSourceInterface(packet);

    EV <<"ProcessSuspend(), suspend from IE name = "<<fromIE->getInterfaceName()<<
            ", ID = "<<fromIE->getInterfaceId()<<", suspend = "<<packet->getPacketECN()<<endl;
    // if suspend map = true, the interface is suspended
    if (packet->getPacketECN() == 1)
    {
        IfSuspendMap[fromIE->getInterfaceId()] = true;
        SendSuspendToMidware(packet);
    }
    else if (packet->getPacketECN() == 0)
    {
        IfSuspendMap[fromIE->getInterfaceId()] = false;
        SendSuspendToMidware(packet);
    }
    else
    {
        throw cRuntimeError("Unspecified suspend packet type.");
    }
}

// added by angrydudu. 12/05/2019
void Ipv4::ProcessPause(Packet *packet)
{
    EV<<"packet scheduling priority = "<<packet->getSchedulingPriority()<<endl;
    // if suspend map = true, the interface is suspended
    if (packet->getSchedulingPriority() == 99)
    {
        BroadcastPause(packet);
    }
    else
    {
        throw cRuntimeError("Unspecified Pause packet type.");
    }
}

void Ipv4::BroadcastPause(Packet *packet)
{
    const InterfaceEntry *fromIE = getSourceInterface(packet);
    EV <<"BroadcastPause(), Pause from IE index = "<<fromIE->getIndex()<<
            ", ID = "<<fromIE->getInterfaceId()<<", Pause = "<<packet->getPacketECN()<<
            ", ift->getNumInterfaces() = "<<ift->getNumInterfaces()<<endl;
    delete packet->removeTagIfPresent<DispatchProtocolReq>();
    for (int i = 1; i < ift->getNumInterfaces(); i++)
    {
        if (i != fromIE->getIndex()+1)
        {
            const InterfaceEntry *ie = ift->getInterface(i);
            EV<<"pause priority = "<<packet->getPriority()<<endl;
            auto packetCopy = packet->dup();
            packetCopy->setPriority(packet->getPriority());
            packetCopy->addTagIfAbsent<InterfaceReq>()->setInterfaceId(ie->getInterfaceId());
            packetCopy->addTagIfAbsent<NextHopAddressReq>()->setNextHopAddress(Ipv4Address::ALLONES_ADDRESS);
            send(packetCopy, "queueOut");
            //fragmentPostRouting(packetCopy);
        }
    }
    delete packet;
...
void Ipv4::RegistMidwareInterface(Packet *packet)
{
    MidwareInterface = getSourceInterface(packet)->getInterfaceId();

    EV <<"RegistMidwareInterface(), Midware interface = "<<ift->getInterfaceById(MidwareInterface)->getInterfaceName()<<
            ", ID = "<<MidwareInterface<<endl;

    delete packet;
}

void Ipv4::SendSuspendToMidware(Packet *packet)
{
    packet->addTagIfAbsent<InterfaceReq>()->setInterfaceId(MidwareInterface);
    delete packet->removeTagIfPresent<DispatchProtocolReq>();
    packet->setPriority(99);
    EV<<"Send suspend to midware, packet ecn =  "<<packet->getPacketECN()<<endl;
...
void Ipv4::decapsulate(Packet *packet)
{
    ...
    EV<<"decapsulate(), ecn = "<<ipv4Header->getExplicitCongestionNotification()<<endl;
...
void Ipv4::encapsulate(Packet *transportPacket)
{
    ...
    if (EcnReq *ecnReq = transportPacket->removeTagIfPresent<EcnReq>()) {
        EV<<"00000000 encapsulate(), ecn has been set, the ecn = "<<ecnReq->getExplicitCongestionNotification()<<endl;
...
void Ipv4::sendPacketToNIC(Packet *packet)
{
    ...
    if (use_midware)
    {
        if (IfSuspendMap.find(packet->findTag<InterfaceReq>()->getInterfaceId()) == IfSuspendMap.end())
        {
            IfSuspendMap[packet->findTag<InterfaceReq>()->getInterfaceId()] = false;
        }

        // if the packet is data packet
        if (string(packet->getFullName()).find("Data") != string::npos ||
                string(packet->getFullName()).find("Msg") != string::npos)
        {
            if (packet->getPriority() > 1 && packet->getPriority() < 10)
            {
                const auto& homapayload = packet->peekAtBack<HomaHeader>();
                if (homapayload->getPktType() == 0 || homapayload->getPktType() == 1)
                {
                    send(packet, "queueOut");
                    EV<<"the packet is "<<packet->getFullName()<<", directly send it to the destination"<<endl;
                    return;
                }
                if (IfSuspendMap.find(packet->findTag<InterfaceReq>()->getInterfaceId())->second)
                {
                    packet->addTagIfAbsent<InterfaceReq>()->setInterfaceId(MidwareInterface);
                    EV<<"send the over-subscribed packet"<<packet->getFullName()<<" to midware"<<endl;
                }
            }
        }
    }
