...
class INET_API Ipv4Route : public cObject, public IRoute
{
  ...
  private:
    ...
    //Ipv4Address GateAddr;    ///< for this hop address
  ...
  public:
    ...
    //virtual void setGateAddr(const Ipv4Address _src){ if (GateAddr != _src) { GateAddr = _src; changed(F_GATEADDR);} } // added by shan huang
    //Ipv4Address getGateAddr() const { return GateAddr; }

    virtual void setecmpIndex(unsigned int _index) { if (ecmpindex != _index) { ecmpindex = _index; changed(F_FINDEX); } }
    unsigned int getecmpIndex() const { return ecmpindex; }

    virtual void setreverseIndex(int _index) { if (reverseindex != _index) { reverseindex = _index; changed(F_BINDEX); } }
    int getreverseIndex() const { return reverseindex; }
     ...
     *
     */
    //Ipv4Address getSrc() const { return src; }

    //virtual void setSrc(Ipv4Address _src) { if (src != _src) { src = _src; changed(F_SRCADDR); } }
