...
class INET_API Ipv4RoutingTable : public cSimpleModule, public IIpv4RoutingTable, protected cListener, public ILifecycle
{
  protected:
    ...
    bool useEcmp = false;          // Added by Shan Huang, for Ecmp routing
    ...
    struct equalInterfaces
    {
        uint32_t ifIndex[256];
        Ipv4Address ifAddr[256];
    };
    struct reverseInterfaces
    {
        int sumifs;
        uint32_t vpid[256];
        uint32_t ifID[256];
    ...
    ecmpRouteVector ecmproutes;    // Unicast route array, sorted by netmask desc, dest asc, metric asc
    // added by shan huang, ecmp routing cache: maps destination address to the route
    typedef std::map<Ipv4Address, Ipv4Route *> ecmpRoutes;
    mutable ecmpRoutes ecmprouteCache;

    typedef std::map<Ipv4Address, Ipv4Route *> reverseRoutes;
    mutable reverseRoutes reverserouteCache;

    typedef std::map<Ipv4Address, int> reverseIFs;
    mutable reverseIFs reverseifCache;

    typedef std::map<Ipv4Address, reverseInterfaces> reverseACIFs;
    mutable reverseACIFs reverseacIFs;

    typedef std::map<Ipv4Address, equalInterfaces> ecmprouteIFs;
  ...
  public:
    ...
    virtual Ipv4Route *findreverseRoute(const Ipv4Address& src,const Ipv4Address& dest, const uint32_t vpid, const bool usemp) const override;

    virtual Ipv4Route *findecmpRoute(const uint32_t ifid_in,const Ipv4Address& src,const Ipv4Address& dest,const uint32_t vpid, const bool usemp) const override;
    virtual void addecmpRoute(uint32_t ecmpIF[256],Ipv4Address efaddr[256],Ipv4Route *entry) override;

    virtual void clearIFs() override {ecmpIFs.clear();}


    //virtual Ipv4Route *findEcmpRoute(Packet *pck) const = 0;
    ...
    virtual int getNumecmpRoutes() const override { return ecmproutes.size(); }


    virtual bool dltecmpRoute(Ipv4Route *entry) override;

    virtual Ipv4Route *getecmpRoute(int k) const override;
  ...
  private:
    ...
    static size_t hashAddresses(const unsigned int src,const unsigned int dest);
