...
{
    routingCache.clear();
    ecmprouteCache.clear(); //added by shan huang
    //ecmpIFs.clear(); //added by shan huang
...
// added by Shan Huang, for ecmp and angrycredits
Ipv4Route *Ipv4RoutingTable::findreverseRoute(const Ipv4Address& src, const Ipv4Address& dest, const uint32_t vpid, const bool usemp) const
{
    Enter_Method("findreverseRoute(%u.%u.%u.%u)", dest.getDByte(0), dest.getDByte(1), dest.getDByte(2), dest.getDByte(3));
    if(usemp)
    {
        EV<<endl<<"findreverseRoute(), using multiple path, find the reverse path of dest = "<<dest<<endl;
        Ipv4Route *bestroute = findBestMatchingRoute(dest);
        auto it = reverseacIFs.find(dest);
        if (it != reverseacIFs.end())
        {
            reverseInterfaces ac_if = it->second;
            int sum_ifs = ac_if.sumifs;
            uint32_t reverse_if = 0;
            for (int i=0;i<sum_ifs;i++)
            {
                if (ac_if.vpid[i] == vpid)
                {
                    reverse_if = ac_if.ifID[i];
                }
            }
            auto rif_it = ecmpIFs.find(bestroute->getDestination());
            if (rif_it != ecmpIFs.end())
            {
                EV<<"findreverseRoute(), find route in ecmproutecache, have not find it in ifIndex yet, reverse if = "<<reverse_if<< endl;
                equalInterfaces equalif = rif_it->second;
                for (unsigned int i=0; i<sum_paths; i++)
                {
                    if (equalif.ifIndex[i] == reverse_if)
                    {
                        bestroute->setInterface(ift->getInterfaceById(equalif.ifIndex[i]));
                        bestroute->setGateway(equalif.ifAddr[i]);
                        EV<<"findreverseRoute(), go to the reverse interface = "<<equalif.ifIndex[i]<<", gateway = "<<equalif.ifAddr[i]<<endl;
                        return bestroute;
                    }
                }
                EV<<"findrecerseRoute(), no reverseRoute, going to use multi-path routing."<<endl;
                return nullptr;
            }
            else
            {
                EV<<"findreverseRoute(), valid interface have not find route in ecmpifcache, return nullptr"<<endl;
                return nullptr;
            }
        }
        else
        {
            EV<<"findreverseRoute(), have not find route in reverseacIFs, return nullptr"<<endl;
            return nullptr;
        }
        return bestroute;
    }
    ...
    {
        EV<<endl<<"findreverseRoute(), without multiple path, find the reverse path of dest = "<<dest<<endl;
        auto reverse_it = reverserouteCache.find(dest);
        if (reverse_it != reverserouteCache.end())
        {
            if (reverse_it->second == nullptr || reverse_it->second->isValid())
            {
                EV<<"findreverseRoute(), find route in reverserouteCache, and return it"<<endl;
                return reverse_it->second;
            }
        }

        Ipv4Route *bestroute = findBestMatchingRoute(dest);
        auto it = reverseifCache.find(dest);
                ...
                if (rif_it != ecmpIFs.end())
                {
                    ...
                    for (unsigned int i=0; i<sum_paths; i++)
                    {
                        if (equalif.ifIndex[i] == reverse_if)
                        {
                            ...
                            bestroute->setGateway(equalif.ifAddr[i]);
                            EV<<"findreverseRoute(), go to the reverse interface = "<<equalif.ifIndex[i]<<", gateway = "<<equalif.ifAddr[i]<<endl;
                            reverserouteCache[dest] = bestroute;
                            return bestroute;
                        }
                    }
                }
                EV<<"findrecerseRoute(), no reverseRoute, going to use ecmp routing."<<endl;
                return nullptr;
            }
            else
            {
        ...
        {
            EV<<"findreverseRoute(), have not find route in ecmproutecache, return nullptr"<<endl;
            return nullptr;
        }
        reverserouteCache[dest] = bestroute;
        return bestroute;
...
// added by shan huang, for ecmp and angrycredits
Ipv4Route *Ipv4RoutingTable::findecmpRoute(const uint32_t ifid_in, const Ipv4Address& src, const Ipv4Address& dest, const uint32_t vpid, const bool usemp) const
{
    Enter_Method("findEcmpRoute(%u.%u.%u.%u)", dest.getDByte(0), dest.getDByte(1), dest.getDByte(2), dest.getDByte(3));
    size_t ecmprouteIndex;

    if(usemp)
    {
        uint32_t temp_id = vpid;
        ecmprouteIndex = (size_t)temp_id;
        EV<<"findecmpRoute(), using multiple path, the index of required path = "<< ecmprouteIndex <<endl;
        Ipv4Route *bestRoute = nullptr;
        for (auto e : ecmproutes) {
            if (e->isValid()) {
                if (Ipv4Address::maskedAddrAreEqual(dest, e->getDestination(), e->getNetmask()))
                {
                    bestRoute = const_cast<Ipv4Route *>(e);
                    EV<<" number of interfaces is = "<<bestRoute->getecmpIndex()<<endl;
                    if (bestRoute->getecmpIndex() == 1)
                    {
                        bestRoute->setInterface(ift->getInterfaceById(ecmpIFs.find(e->getDestination())->second.ifIndex[0]));
                        bestRoute->setGateway(ecmpIFs.find(e->getDestination())->second.ifAddr[0]);
                        EV_INFO << "findecmpRoute(), the hashed index to interface = "<< bestRoute->getInterfaceName() <<endl;
                        break;
                    }
                    else
                    {
                        //unsigned int ifid = (unsigned int)ecmprouteIndex % bestRoute->getecmpIndex();
                        uint32_t ifid = ecmprouteIndex % sum_paths;
                        EV_INFO << "findecmpRoute(), the ifid = "<< ifid <<", in ecmpIFs = "<<ecmpIFs.find(e->getDestination())->second.ifIndex[ifid]<<endl;
                        bestRoute->setInterface(ift->getInterfaceById(ecmpIFs.find(e->getDestination())->second.ifIndex[ifid]));
                        bestRoute->setGateway(ecmpIFs.find(e->getDestination())->second.ifAddr[ifid]);
                        EV_INFO << "findecmpRoute(), the hashed index to interface = "<< bestRoute->getInterfaceName() <<endl;
                        break;
                    }
                }
        ...
        // find best match (one with longest prefix)
        // default route has zero prefix length, so (if exists) it'll be selected as last resort
        if (ifid_in != 0) // = 0, the route is searched by the findreverseroute()
        {
            auto ac_it = reverseacIFs.find(src);
            if (ac_it != reverseacIFs.end()) // not the first reverse path of this src.
            {
                reverseInterfaces rvifs = ac_it->second;
                int sum_ifs = rvifs.sumifs;
                bool existed = false;
                for(int i=0;i<sum_ifs;i++)
                {

                    EV<<"findecmpRoute(), 11111111 vpid = "<<rvifs.vpid[i]<<", mine = "<<vpid<<endl;
                    if (rvifs.vpid[i]==vpid)
                        existed = true;
                }
                if (!existed)
                {
                    rvifs.ifID[rvifs.sumifs] = ifid_in;
                    rvifs.vpid[rvifs.sumifs] = vpid;
                    rvifs.sumifs = rvifs.sumifs + 1;
                    EV<<"findecmpRoute(), "<< src <<" new reverse interface has been set = "<< ifid_in <<", sumifs = "<<rvifs.sumifs<<endl;
                    reverseacIFs[src] = rvifs;
                }
            }
            else // the first reverse path of this src.
            {
                reverseInterfaces rvifs;
                rvifs.ifID[0] = ifid_in;
                rvifs.vpid[0] = vpid;
                rvifs.sumifs = 1;
                EV<<"findecmpRoute(), "<< src <<" new reverse interface has been set = "<< ifid_in <<", sumifs = "<<rvifs.sumifs<<endl;
                reverseacIFs[src] = rvifs;
    ...
    {
        ecmprouteIndex = hashAddresses((unsigned int)src.getInt(),(unsigned int)dest.getInt());
        EV<<"findecmpRoute(), using single path, the index of required path = "<< ecmprouteIndex <<endl;
        auto it = ecmprouteCache.find(dest);
        if (it != ecmprouteCache.end())
        {
            if (it->second == nullptr || it->second->isValid())
            {
                Ipv4Route *ecmproute = it->second;
                EV<<"findecmpRoute(), using ecmpRouteCache, interface has been already set, return the route."<<endl;
                return ecmproute;
            }
        }
        Ipv4Route *bestRoute = nullptr;
            ...
            if (e->isValid()) {
                if (Ipv4Address::maskedAddrAreEqual(dest, e->getDestination(), e->getNetmask()))
                {
                    ...
                    if (bestRoute->getecmpIndex() == 1)
                    {
                        bestRoute->setInterface(ift->getInterfaceById(ecmpIFs.find(e->getDestination())->second.ifIndex[0]));
                        bestRoute->setGateway(ecmpIFs.find(e->getDestination())->second.ifAddr[0]);
                        EV_INFO << "ipv4routingtable.cc, the hashed index to interface = "<< bestRoute->getInterfaceName() <<endl;
                        break;
                    }
                    else
                    {
                        uint32_t ifid = ecmprouteIndex % bestRoute->getecmpIndex();
                        EV_INFO << "ipv4routingtable.cc, the ifid = "<< ifid <<", in ecmpIFs = "<<ecmpIFs.find(e->getDestination())->second.ifIndex[ifid]<<endl;
                        bestRoute->setInterface(ift->getInterfaceById(ecmpIFs.find(e->getDestination())->second.ifIndex[ifid]));
                        bestRoute->setGateway(ecmpIFs.find(e->getDestination())->second.ifAddr[ifid]);
                        EV_INFO << "ipv4routingtable.cc, the hashed index to interface = "<< bestRoute->getInterfaceName() <<endl;
                        break;
        ...
        if (ifid_in != 0) // = 0, the route is searched by the findreverseroute()
        {
            EV<<"findecmpRoute(), "<< src <<" reverse interface has been set = "<< ifid_in <<endl;
            reverseifCache[src] = ifid_in;
        }
        ecmprouteCache[dest] = bestRoute;
        routingCache[dest] = bestRoute;
        return bestRoute;
    }
}

// added by shan huang, for ecmp
void Ipv4RoutingTable::addecmpRoute(uint32_t ecmpIF[256], Ipv4Address efaddr[256], Ipv4Route *entry)
{
    Enter_Method("addecmpRoute(...)");

    equalInterfaces equalIFs;

    unsigned int sumif = 0;
    unsigned int i;
    EV_INFO << "ipv4routingtable.cc, add ecmp route " << entry->str() <<endl;
    for (i = 0;i<sum_paths;i++)
    {
        if (ecmpIF[i] != 0)
        {
            equalIFs.ifIndex[i] = ecmpIF[i];
            equalIFs.ifAddr[i] = efaddr[i];
            EV<<"addecmproute(), set interfaceID = "<<equalIFs.ifIndex[i]<<endl;
        }
        else
        {
            break;
    ...
    sumif = i;
    entry->setecmpIndex(sumif);
    ecmpIFs[entry->getDestination()] = equalIFs;
    ecmproutes.push_back(entry);
    EV<<"addecmproute(), equalIFs = "<<sumif<<endl;
    invalidateCache();
...
bool Ipv4RoutingTable::dltecmpRoute(Ipv4Route *entry)
{
    Enter_Method("dltecmpRoute(...)");

    auto i = std::find(ecmproutes.begin(), ecmproutes.end(), entry);
    if (i != ecmproutes.end()) {
        ecmproutes.erase(i);
        invalidateCache();
    }
    ...
    if (entry != nullptr) {
        EV_INFO << "delete ecmp route " << entry->str() << "\n";
        //ASSERT(entry->getRoutingTable() == this);    // still filled in, for the listeners' benefit
        emit(routeDeletedSignal, entry);
        delete entry;
    }
...
Ipv4Route *Ipv4RoutingTable::getecmpRoute(int k) const
{
    if (k < (int)ecmproutes.size())
        return ecmproutes[k];
    return nullptr;
}
...
size_t Ipv4RoutingTable::hashAddresses(const unsigned int src,const unsigned int dest)
{
    size_t value1 = hash<unsigned int>{}(src/23);
    size_t value2 = hash<unsigned int>{}(dest/29);
    return hash<unsigned int>{}((value1+value2));
}
} // namespace inet
