...
class INET_API IIpv4RoutingTable : public IRoutingTable
{
  public:
    ...
    virtual bool dltecmpRoute(Ipv4Route *entry) = 0;

    virtual Ipv4Route *getecmpRoute(int k) const = 0;

    virtual int getNumecmpRoutes() const = 0;


    virtual Ipv4Route *findreverseRoute(const Ipv4Address& src,const Ipv4Address& dest,const uint32_t vpid,const bool usemp) const = 0;

    virtual Ipv4Route *findecmpRoute(const uint32_t ifid_in,const Ipv4Address& src,const Ipv4Address& dest,const uint32_t vpid,const bool usemp) const = 0;

    virtual void addecmpRoute(uint32_t ecmpIF[256],Ipv4Address efaddr[256],Ipv4Route *entry) = 0;

    virtual void clearIFs() = 0;


    //using IRoutingTable::findecmpRoute;
