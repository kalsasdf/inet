...
#include <random>
#include <cmath>
#include "inet/homa/transport/PriorityResolver.h"
 ...
 */
/*
simsignal_t HomaTransport::LeftMsgIDSignal =
        registerSignal("leftMsgIDs");
simsignal_t HomaTransport::LeftMsgSizeSignal =
        registerSignal("LeftMsgSize");
simsignal_t HomaTransport::LeftMsgQueueSignal =
        registerSignal("LeftQueueSize");
simsignal_t HomaTransport::LeftMsgTxPckSignal =
        registerSignal("LeftMsgTxPcks");
simsignal_t HomaTransport::LeftMsgsMapSignal =
        registerSignal("LeftMsgsMapSize");
simsignal_t HomaTransport::RcvSenderMapSizeSignal =
        registerSignal("RcvSenderMapSize");
simsignal_t HomaTransport::RcvMsgToGrantSignal =
        registerSignal("RcvMsgToGrant");
...
HomaTransport::registerTemplatedStats()
{
    ...
    for (size_t i = 0; i < numPrio; i++) {
        ...
        //getEnvir()->addResultRecorders(this, pktPrioSignal, pktPrioStatsName,
        //        statisticTemplate);
    }
}
...
HomaTransport::initialize()
{
    ...
    //throw cRuntimeError("gate error");
...
HomaTransport::processStart()
{
    ...
    socket.setOutputGate(gate("lowerOut"));
...
HomaTransport::handleMessage(cMessage *msg)
{
    ...
    if (msg->isSelfMessage()) {
        ...
        if (msg->arrivedOn("upperIn")) {
            ...
            EV<<"msg from upper layer "<<outMsg->getFullName()<<endl;
            EV<<"Messages left to send, outboundMsgMap size = "<<sxController.outboundMsgMap.size()<<
                    ", outboundMsgSet size = "<<sxController.outbndMsgSet.size()<<endl;
            sxController.processSendMsgFromApp(outMsg);
        } else if (msg->arrivedOn("lowerIn")) {
            EV<<"pck priority = "<<check_and_cast<Packet*>(msg)->getPriority()<<", path id = "<<
                    check_and_cast<Packet*>(msg)->getPathID()<<", packet ecn = "<<
                    check_and_cast<Packet*>(msg)->getPacketECN()<<endl;
            HomaPkt *homapck = convertPacket2HomaPkt(check_and_cast<Packet*>(msg));
            L3Address srcaddr = rcv_MsgId_SrcAddr.find(homapck->getMsgId())->second;
            //assert(srcaddr == homapck->getSrcAddr());
            EV<<"msg from lower layer, homa header priority = "<<homapck->getPriority()<<
                    ", msgID = "<<homapck->getMsgId()<<", received packet type = "<<homapck->getPktType()<<endl;
            handleRecvdPkt(check_and_cast<cPacket*>(homapck));
        }
    }
}
...
HomaTransport::handleRecvdPkt(cPacket* pkt)
{
    ...
    EV<<"the received packet type = "<<rxPkt->getPktType()<<endl;
    // handle data or grant packets appropriately
    switch (rxPkt->getPktType()) {
        case PktType::CNP:
            EV<<"entering cnp"<<endl;
            sxController.processReceivedCNP(rxPkt);
            break;
        case PktType::REQUEST:
        case PktType::UNSCHED_DATA:
            EV<<"entering unsched_data"<<endl;
        case PktType::SCHED_DATA:
            EV<<"entering sched_data"<<endl;
            rxScheduler.processReceivedPkt(rxPkt);
            break;

        case PktType::GRANT:
            EV<<"entering grant"<<endl;
    ...
    if (rxScheduler.getInflightBytes() == 0) {
        ...
        //emit(rxActiveBytesSignal, (unsigned long)rxScheduler.rcvdBytesPerActivePeriod);
        rxScheduler.rcvdBytesPerActivePeriod = 0;
...
{
/*
    auto it = sxController.outboundMsgMap.begin();
    for (; it!=sxController.outboundMsgMap.end(); ++ it)
    {
        OutboundMessage *outMsg = &it->second;
        emit(LeftMsgIDSignal,(unsigned long)outMsg->getMsgId());
        emit(LeftMsgSizeSignal,(unsigned long)outMsg->getBytesLeft());
        emit(LeftMsgQueueSignal,outMsg->getTxPktQueue().size());
        emit(LeftMsgTxPckSignal,outMsg->getTxSchedPkts().size());
    }

    auto rcvIt = rxScheduler.ipSendersMap.begin();
    for (; rcvIt!=rxScheduler.ipSendersMap.end(); ++ rcvIt)
    {
        ReceiveScheduler::SenderState *sed = rcvIt->second;
        emit(RcvSenderMapSizeSignal,sed->incompleteMesgs.size());
        emit(RcvMsgToGrantSignal,sed->mesgsToGrant.size());
    }
    emit(LeftMsgsMapSignal, (unsigned long)sxController.outboundMsgMap.size());
    emit(msgsLeftToSendSignal, (unsigned long)sxController.outbndMsgSet.size());
    emit(bytesLeftToSendSignal, (unsigned long)sxController.bytesLeftToSend);
    emit(bytesNeedGrantSignal, (unsigned long)sxController.bytesNeedGrant);
    emit(totalOutstandingBytesSignal,(unsigned long)rxScheduler.getInflightBytes());
    emit(outstandingGrantBytesSignal,rxScheduler.transport->outstandingGrantBytes);
...
HomaPkt *
HomaTransport::convertPacket2HomaPkt(Packet* pck)
{
    HomaPkt *homapck = new HomaPkt(rxScheduler.transport,pck->getFullName());
    const auto& homapayload = pck->peekAtBack<HomaHeader>();
    homapck->setMsgId(homapayload->getMsgId());
    homapck->setBitLength(homapayload->getBitLen());
    homapck->setGrantFields(homapayload->getGrantFields());
    homapck->setSchedDataFields(homapayload->getSchedDataFields());
    homapck->setUnschedFields(homapayload->getUnschedFields());
    homapck->setPriority(homapayload->getPriority());
    homapck->setDestAddr(homapayload->getDestAddr());
    homapck->setSrcAddr(homapayload->getSrcAddr());
    homapck->setPktType(homapayload->getPktType());
    homapck->setECN(pck->getPacketECN());

    /*
     * disable these codes to disable CNP packets.
    if (string(pck->getFullName()).find("Homa") != string::npos && string(pck->getFullName()).find("Grant") == string::npos  )
    {
        sendCongestionNotification(homapayload->getDestAddr(),homapayload->getSrcAddr(),pck->getPacketECN());
    }
    */

    EV<<"srcaddr = "<<homapayload->getSrcAddr()<<", destaddr = "<<homapayload->getDestAddr()<<
            ", bit length = "<<homapayload->getBitLen()<<", creation time = "<<homapck->getUnschedFields().msgCreationTime<<endl;
    delete pck;
    //delete homapayload;
    return homapck;
}
...
void
HomaTransport::sendCongestionNotification(L3Address cnp_srcaddr, L3Address cnp_destaddr, uint32_t ecn)
{
    Packet *cnPacket = new Packet("CNP");
    const auto& homaheader = makeShared<HomaHeader>();
    homaheader->setPktType(CNP);
    homaheader->setSrcAddr(cnp_srcaddr);
    homaheader->setDestAddr(cnp_destaddr);
    homaheader->setChunkLength(b(64));
    homaheader->setPriority(99);
    cnPacket->setPathID(getrandpathid());
    cnPacket->setPriority(99);
    cnPacket->setPacketECN(ecn);
    cnPacket->insertAtBack(homaheader);
    socket.sendTo(cnPacket,cnp_destaddr,homaConfig->destPort);
}

Packet *
HomaTransport::SendController::convertHomaPkt2Packet(HomaPkt* homapck)
{
    EV_STATICCONTEXT;
    Packet *pck = new Packet(homapck->getFullName());

    //pck->setByteLength(homapck->getDataBytes()>>3);//setBitLength(homapck->getBitLength());
    const auto& homapayload = makeShared<HomaHeader>();
    homapayload->setMsgId(homapck->getMsgId());
    homapayload->setBitLen(homapck->getBitLength());
    homapayload->setChunkLength(b(homapck->getBitLength()));
    homapayload->setGrantFields(homapck->getGrantFields());
    homapayload->setSchedDataFields(homapck->getSchedDataFields());
    homapayload->setUnschedFields(homapck->getUnschedFields());
    homapayload->setPriority(homapck->getPriority());
    homapayload->setDestAddr(homapck->getDestAddr());
    homapayload->setSrcAddr(homapck->getSrcAddr());
    homapayload->setPktType(homapck->getPktType());
    homapayload->setECN(homapck->getECN());
    pck->insertAtBack(homapayload);
    pck->setPriority(homapck->getPriority());
    pck->setPathID(transport->getrandpathid());
    pck->setPacketECN(homapck->getECN());
    EV<<"send pck, the priority = "<<pck->getPriority()<<", path id = "<<pck->getPathID()<<endl;
    //delete homapck;
    return pck;
}
...
HomaTransport::SendController::handlePktTransmitEnd()
{
    ...
    // related those transmission delays.
 ...
 *
 * \param sendMsg
 *      Application message that needs to be transmitted by the transport.
 */
void
HomaTransport::SendController::processSendMsgFromApp(AppMessage* sendMsg)
{
    if (outbndMsgSet.size()!=outboundMsgMap.size())
    {
        //throw cRuntimeError("ProcessSendMsgFromApp() 1: outbndmsgset %i != outboundmsgmap %i !",outbndMsgSet.size(),outboundMsgMap.size());
    }
    //transport->emit(LeftMsgsMapSignal, (unsigned long)outboundMsgMap.size());
    //transport->emit(msgsLeftToSendSignal, (unsigned long)outbndMsgSet.size());
    //transport->emit(bytesLeftToSendSignal, (unsigned long)bytesLeftToSend);
    //transport->emit(bytesNeedGrantSignal, (unsigned long)bytesNeedGrant);
    ...
    char msgName[100];
    sprintf(msgName, "HomaData-%s",const_cast<char*>(sendMsg->getFullName()));

    msgId = msgID;
    // initialize the new message for its outboundMsg.
    outboundMsgMap.emplace(std::piecewise_construct,
            std::forward_as_tuple(msgId),
            std::forward_as_tuple(sendMsg, this, msgId, msgName, reqUnschedDataVec));

    OutboundMessage* outboundMsg = &(outboundMsgMap.at(msgId));

    // added by angrydudu, for ecn flow control
    PaceInfo _paceinfo;
    _paceinfo.pace_factor = 1;
    _paceinfo.ecnbitmap = 0;
    destsPaceInfoMap[sendMsg->getDestAddr()] = _paceinfo;
    ...
    if (outbndMsgSet.size()!=outboundMsgMap.size())
    {
        //throw cRuntimeError("ProcessSendMsgFromApp() 2: outbndmsgset %i != outboundmsgmap %i !",outbndMsgSet.size(),outboundMsgMap.size());
    }
    //ASSERT(outbndMsgSet.size()==outboundMsgMap.size());
...
/**
 * This method handles received CNP packet to update the pace factor
 */
void
HomaTransport::SendController::processReceivedCNP(HomaPkt* rxPkt)
{
    EV_STATICCONTEXT;
    auto it = destsPaceInfoMap.find(rxPkt->getSrcAddr());
    if(it == destsPaceInfoMap.end())
    {
        return;
    }
    PaceInfo _paceinfo = it->second;
    EV<<"old _paceinfo.ecnbitmap = "<<(short)_paceinfo.ecnbitmap<<
            ", pace_factor = "<<_paceinfo.pace_factor<<endl;
    _paceinfo.ecnbitmap = _paceinfo.ecnbitmap<<1;
    if (rxPkt->getECN() == 3)
    {
        _paceinfo.ecnbitmap += 1;
    }
    int count = 0;
    uint8_t temp_bitmap = _paceinfo.ecnbitmap;
    while (temp_bitmap) {
        if (temp_bitmap & 0x1)
            count++;
        temp_bitmap >>= 1;
    }
    _paceinfo.pace_factor = 1-((double)count/8)/2;
    destsPaceInfoMap[rxPkt->getSrcAddr()] = _paceinfo;
    EV<<"after updating based on the received CNP, "
            "new _paceinfo.ecnbitmap = "<<(short)_paceinfo.ecnbitmap<<
            ", pace_factor = "<<_paceinfo.pace_factor<<endl;
}
 ...
 *
 * \param rxPkt
 *      received grant packet from the network.
 */
void
HomaTransport::SendController::processReceivedGrant(HomaPkt* rxPkt)
{
    ...
    {
        EV<<"No corresponding outboundMsg for this grant."<<endl;
        delete rxPkt;
        return;
    }
    OutboundMessage* outboundMsg = &(outboundMsgMap.at(rxPkt->getMsgId()));

    //OutboundMessage* outboundMsg = &(outboundMsgMap.find(rxPkt->getMsgId())->second);
    ASSERT(rxPkt->getPktType() == PktType::GRANT &&
        rxPkt->getDestAddr() == outboundMsg->srcAddr &&
        rxPkt->getSrcAddr() == outboundMsg->destAddr);

    //throw cRuntimeError("the msgId = d, packet name = %s", outboundMsg->pckName.c_str());
    size_t numRemoved = outbndMsgSet.erase(outboundMsg);
    if (numRemoved > 1)
    {
        throw cRuntimeError("the amount of erased msges = %i",numRemoved);
    }
    ASSERT(numRemoved <= 1); // At most one item removed

    //outbndMsgSet.insert(__v);
    ...
    if (outbndMsgSet.size()!=outboundMsgMap.size())
    {
        //throw cRuntimeError("ProcessReceivedGrant():  outbndmsgset %i != outboundmsgmap %i !",outbndMsgSet.size(),outboundMsgMap.size());
    }
    //ASSERT(outbndMsgSet.size()==outboundMsgMap.size());
    delete rxPkt;
    sendOrQueue();
...
HomaTransport::SendController::sendOrQueue(cMessage* msg)
{
    ...
    if (msg == transport->sendTimer) {
        ...
        if (!outbndMsgSet.empty()) {
            ...
            //sxPkt->setName(highPrioMsg->getPckName());
            sendPktAndScheduleNext(sxPkt);
            if (highPrioMsg->getBytesLeft() <= 0) {
                if (hasMoreReadyPkt)
                    throw cRuntimeError("SendOrQueue(): no bytes left but exists more ready pkt!");
    ...
    if (transport->sendTimer->isScheduled()) {
        if (outbndMsgSet.size()!=outboundMsgMap.size())
        {
            //throw cRuntimeError("SendOrQueue(): sendTimer is scheduled, outbndmsgset %i != outboundmsgmap %i !",
            //        outbndMsgSet.size(),outboundMsgMap.size());
        }
        return;
    }
    ...
    //else {
        ...
        if (outbndMsgSet.size()!=outboundMsgMap.size())
        {
            //throw cRuntimeError("SendOrQueue(): outbndmsgset %i != outboundmsgmap %i !",outbndMsgSet.size(),outboundMsgMap.size());
        }
        return;
    }
}
...
HomaTransport::SendController::sendPktAndScheduleNext(HomaPkt* sxPkt)
{
    ...
    EV<<"send packet, numDataBytes = "<<numDataBytes<<", bytesSentOnWire = "<<bytesSentOnWire<<
            ", byteLen = "<<sxPkt->getByteLength()<<endl;
    switch(pktType)
    {
        case PktType::REQUEST:
        case PktType::UNSCHED_DATA:
        {
            OutboundMessage* thisMsg = &(outboundMsgMap.at(sxPkt->getMsgId()));
            thisMsg->nextExpectedUnschedTxTime = thisMsg->nextExpectedUnschedTxTime + sxPktDuration;
            EV<<"next expected unschedule Tx time = "<<thisMsg->nextExpectedUnschedTxTime<<endl;
        }
        case PktType::SCHED_DATA:
            ...
            if (bytesLeftToSend == 0) {
                ...
                //    sxPktDuration;
                //transport->emit(sxActiveBytesSignal, (unsigned long)sentBytesPerActivePeriod);
    ...
    sentPkt = *sxPkt;
    //sxPkt->setName("HomaPacket");
    sentPktDuration = sxPktDuration;
    transport->socket.sendTo(convertHomaPkt2Packet(sxPkt), sxPkt->getDestAddr(), homaConfig->destPort);
    delete sxPkt;
    //transport->socket.sendTo(check_and_cast<cPacket*>(sxPkt), sxPkt->getDestAddr(), homaConfig->destPort);
    transport->scheduleAt(nextSendTime, transport->sendTimer);
}

/**
 * This method cleans up a message whose transmission is complete.
 *
 * \param msg
...
HomaTransport::SendController::msgTransmitComplete(OutboundMessage* msg)
{
    ...
    if (rxAddrMsgMap.at(destIp).empty()) {
        ...
        destsPaceInfoMap.erase(msg->destAddr);
    }
    ...
    if (outbndMsgSet.size()!=outboundMsgMap.size())
    {
        //throw cRuntimeError("msgTransmitComplete(): outbndmsgset %i != outboundmsgmap %i !",outbndMsgSet.size(),outboundMsgMap.size());
    }
    transport->testAndEmitStabilitySignal();
    return;
}
...
HomaTransport::SendController::OutbndMsgSorter::operator()(
        OutboundMessage* msg1, OutboundMessage* msg2) const {
...
/**
 * Constructor for HomaTransport::OutboundMessage.
 *
 * \param outMsg
...
HomaTransport::OutboundMessage::OutboundMessage(AppMessage* outMsg,
        SendController* sxController, uint64_t msgId, string pckName,
        std::vector<uint16_t> reqUnschedDataVec)
    : msgId(msgId)
    , pckName(pckName)
    ...
    , unschedBytesLeft(0)
    , nextExpectedUnschedTxTime(simTime())
...
HomaTransport::OutboundMessage::prepareRequestAndUnsched()
{
    ...
    do {
        ...
        unschedPkt->setName(this->pckName.c_str()); // added by angrydudu 191029
        //throw cRuntimeError("the msgId = %d, packet name = %s",this->msgId,this->pckName);
    ...
    uint16_t numBytes, uint16_t schedPrio)
{
    ...
    dataPkt->setName(this->pckName.c_str());
    //throw cRuntimeError("the msgId = %d, packet name = %s",this->msgId,this->pckName.c_str());
    //dataPkt->setName(str.str().c_str()); // added by angrydudu 191029
...
HomaTransport::ReceiveScheduler::lookupInboundMesg(HomaPkt* rxPkt) const
{
    ...
    //auto iter = ipSendersMap.find(rxPkt->getMsgId());
...
HomaTransport::ReceiveScheduler::processReceivedPkt(HomaPkt* rxPkt)
{
    ...
    EV<<"pktLenOnWire = "<<pktLenOnWire<<", sinceLastPkt = "<<sinceLastPkt<<", pktDuration = "<<pktDuration<<endl;
    ...
    *sumGrantsInGap = 0;
    ...
    if (schedSenders->numSenders > schedSenders->numToGrant) {
        ...
        ASSERT(inOversubPeriod && oversubPeriodStop==SIMTIME_MAX &&
        ...
        //transport->emit(oversubBytesSignal, (unsigned long)rcvdBytesPerOversubPeriod);

        // reset the states and variables
        rcvdBytesPerOversubPeriod = 0;
        oversubPeriodStart = SIMTIME_MAX;
    ...
    if (iter == ipSendersMap.end()) {
        ...
        char timerName[50];
        sprintf(timerName, "sender-IP-%s", srcIp.str().c_str());
        ...
        transport->rcv_MsgId_SrcAddr[rxPkt->getMsgId()] = rxPkt->getSrcAddr();
    } else {
        s = iter->second;
    }
    ...
    // transmitted. if message complete this time, the cur.sInd = -1;
    ...
    // Each new data packet arrival is a hint that receive link is being
...
HomaTransport::ReceiveScheduler::SenderState::SenderState(
        Ipv4Address srcAddr, ReceiveScheduler* rxScheduler,
...
HomaTransport::ReceiveScheduler::SenderState::handleInboundPkt(HomaPkt* rxPkt)
{
    ...
    if (mesgIt == incompleteMesgs.end()) {
        ...
        inboundMesg = new InboundMessage(rxPkt, this->rxScheduler, homaConfig, const_cast<char*>(rxPkt->getFullName()));
        ...
        const std::vector<uint32_t>& prioUnschedBytes =
        ...
        //        (unsigned long)rxScheduler->getInflightBytes());

        for (size_t i = 0; i < rxPkt->getUnschedFields().prioUnschedBytes.size(); i += 2) {
            uint32_t prio = rxPkt->getUnschedFields().prioUnschedBytes[i];
            uint32_t unschedBytesInPrio = rxPkt->getUnschedFields().prioUnschedBytes[i+1];
            rxScheduler->addPendingUnschedBytes(pktType, prio,
                unschedBytesInPrio);
        }
        ...
        //    totalOutstandingBytesSignal, (unsigned long)rxScheduler->getInflightBytes());
    ...
    if (inboundMesg->bytesToReceive == 0) {
        ...
        //rxScheduler->transport->emit(RcvSenderMapSizeSignal,incompleteMesgs.size());
        //rxScheduler->transport->emit(RcvMsgToGrantSignal,mesgsToGrant.size());
        ...
        delete rxMsg;
#else
        //EV<<"The message creation time = "<<rxMsg->getCreationTime()<<endl;
        rxScheduler->transport->send(rxMsg, "upperOut", 0);
#endif
        ...
        // need to keep the message around until all of its packets are arrived.
        if (isMesgSched) {
            ret.second = 0;
        } else {
        ...
        uint32_t grantPrio)
{
    ...
    EV<<"sendAndScheduleGrant(): topMesg msg name = "<<topMesg->pckNameAtSender<<endl;
    ...
    if (grantTimer->isScheduled()){
        if (lastGrantPrio <= grantPrio) {
            EV<<"sendAndScheduleGrant() 1: lastGrantPrio = "<<lastGrantPrio<<", grantPrio = "<<lastGrantPrio<<endl;
            return 0;
        }
    ...
    if (topMesg->totalBytesInFlight >= homaConfig->maxOutstandingRecvBytes) {
        EV<<"sendAndScheduleGrant() 2: topMesg->totalBytesInFlight = "<<topMesg->totalBytesInFlight<<
                ", maxOutstandingRecvBytes = "<<homaConfig->maxOutstandingRecvBytes<<endl;
        return 0;
    }
    ...
    if (mesgsToGrant.empty()) {
        ...
        EV<<"sendAndScheduleGrant() 3: mesgsToGrant is empty!"<<endl;
        return -1;
    }
 ...
 * of this method invocation is that it also inserts the SenderState into the
...
HomaTransport::InboundMessage::InboundMessage(HomaPkt* rxPkt,
        ReceiveScheduler* rxScheduler, HomaConfigDepot* homaConfig, string pckNameAtSender)
    : rxScheduler(rxScheduler)
    , homaConfig(homaConfig)
    , pckNameAtSender(pckNameAtSender)
        ...
        uint32_t byteEnd, PktType pktType)
{
    ...
    if (bytesToReceive>4200000000)
    {
        bytesToReceive = 0;
    }
    if (totalBytesInFlight>4200000000)
    {
        totalBytesInFlight = 0;
    }
    ...
    if (pktType == PktType::UNSCHED_DATA || pktType == PktType::REQUEST) {
        ...
        EV<<"std::get<0>(*grantZero) = "<<std::get<0>(*grantZero)<<", std::get<1>(*grantZero)"<<std::get<1>(*grantZero)
                <<", bytesReceived = "<<bytesReceived<<endl;
    ...
    uint16_t schedPrio)
{
    ...
    // prepare a grant
    char msgName[100];
    sprintf(msgName, "Grant-%s",this->pckNameAtSender.c_str());
    HomaPkt* grantPkt = new HomaPkt(rxScheduler->transport,msgName);
    ...
    grantPkt->setPriority(99);

    EV<<"offset = "<<offset<<", Grant size = "<<grantSize<<endl;

    uint32_t grantedBytesOnWire = HomaPkt::getBytesOnWire(grantSize,
        PktType::SCHED_DATA);
...
HomaTransport::InboundMessage::prepareRxMsgForApp()
{
    ...
    rxMsg->setMsgId(this->msgIdAtSender);
...
HomaTransport::InboundMessage::copy(const InboundMessage& other)
{
    ...
    this->homaConfig = other.homaConfig;
    this->pckNameAtSender = other.pckNameAtSender;
    this->srcAddr = other.srcAddr;
    this->destAddr = other.destAddr;
