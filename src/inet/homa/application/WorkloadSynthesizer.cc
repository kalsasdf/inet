...
WorkloadSynthesizer::initialize()
{
    ...
    nicLinkSpeed = long(par("nicLinkSpeed").doubleValue());
    fabricLinkSpeed = long(par("fabricLinkSpeed").doubleValue());
    ...
    HomaPkt HomaPck = HomaPkt();
    HomaPck.setPktType(PktType::UNSCHED_DATA);
    maxDataBytesPerEthFrame =
            MAX_ETHERNET_PAYLOAD_BYTES - IP_HEADER_SIZE - UDP_HEADER_SIZE;
    maxDataBytesPerPkt = maxDataBytesPerEthFrame - HomaPck.headerSize();
...
WorkloadSynthesizer::parseAndProcessXMLConfig()
{
    ...
    while((token = tokenizer.nextToken()) != NULL) {
        cModule* mod = getModuleByPath(token);
        L3Address result;
...
WorkloadSynthesizer::idealMsgEndToEndDelay(AppMessage* rcvdMsg)
{
    ...
    L3Address srcAddr = rcvdMsg->getSrcAddr();
    ASSERT(srcAddr.getType() == L3Address::AddressType::IPv4);
    L3Address destAddr = rcvdMsg->getDestAddr();
    ASSERT(destAddr.getType() == L3Address::AddressType::IPv4);
