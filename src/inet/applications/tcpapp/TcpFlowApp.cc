...
TcpFlowApp::~TcpFlowApp()
{
    cancelAndDelete(timeoutMsg);
}

void TcpFlowApp::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        avgBytes = par("avgBytes");
        avgTimeGap = par("avgTimeGap");
        randomSending = par("randomSending");
        sumFlows = par("sumFlows");
        byteJitter = par("byteJitter");
        byte_seed = par("byteSeed");
        time_seed = par("timeSeed");
        timeJitter = par("timeJitter");
        commandIndex = 0;
        ...
        if (randomSending)
        {
            ...
            {
                for (int i=1;i<=sumFlows;i++)
                {
                    srand(byte_seed*i);
                    double _byte = rand()%10000;
                    sendBytes = avgBytes - byteJitter + _byte/10000*(byteJitter*2);
                    srand(time_seed*i);
                    double _time = rand()%(int(timeJitter*1000000*2));
                    tSend = tSend + avgTimeGap - timeJitter + _time/1000000;
...
bool TcpFlowApp::isNodeUp()
{
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

bool TcpFlowApp::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void TcpFlowApp::handleTimer(cMessage *msg)
{
    switch (msg->getKind()) {
...
Packet *TcpFlowApp::createDataPacket(long sendBytes)
{
...
void TcpFlowApp::socketEstablished(TcpSocket *socket)
{
...
void TcpFlowApp::socketDataArrived(TcpSocket *socket, Packet *msg, bool urgent)
{
...
void TcpFlowApp::socketClosed(TcpSocket *socket)
{
    TcpAppBase::socketClosed(socket);
    cancelEvent(timeoutMsg);
}

void TcpFlowApp::socketFailure(TcpSocket *socket, int code)
{
    TcpAppBase::socketFailure(socket, code);
    cancelEvent(timeoutMsg);
}

void TcpFlowApp::parseScript(const char *script)
{
    const char *s = script;
...
void TcpFlowApp::refreshDisplay() const
{
