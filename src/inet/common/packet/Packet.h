...
class INET_API Packet : public cPacket
{
  ...
  public:
    ...
    uint32_t getPacketECN() const {return PacketECN;}

    uint32_t getFlowId() const {return FlowId;}

    void setPacketECN(uint32_t ecn) {PacketECN = ecn;}

    void setPathID(uint32_t id) {PathID = id;}

    void setPriority(uint32_t prio) {Priority = prio;}

    void setFlowId(uint32_t flowid) {FlowId = flowid;}
