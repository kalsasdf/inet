...
void EtherMacBase::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        endSuspendTxMsg = new cMessage("EndSuspendTx",ENDSUSPENDTX);
...
bool EtherMacBase::dropFrameNotForUs(Packet *packet, const Ptr<const EthernetMacHeader>& frame)
{
    ...
    EV<< "0000 the packet's and interface mac addresses are"<<frame->getDest()<<" vs "<<address<<endl;
    numDroppedNotForUs++;
    PacketDropDetails details;
...
void EtherMacBase::getNextFrameFromQueue()
{
    ...
    if (txQueue.extQueue) {
        EV<<"entering getNextFrameFromQueue(), getNumPendingRequests() = "<<txQueue.extQueue->getNumPendingRequests()<<endl;
        if (txQueue.extQueue->getNumPendingRequests() == 0)
        {
            txQueue.extQueue->requestPacket();
        }
