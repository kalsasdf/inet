...
void EtherMacFullDuplex::handleMessage(cMessage *msg)
{
    ...
    else if (msg->getArrivalGate() == upperLayerInGate)
    {
        ...
        {
            if (check_and_cast<Packet*>(msg)->getPriority() == 99)
            {
                //processFrameFromUpperLayer(check_and_cast<Packet *>(msg));
                curTxFrame = check_and_cast<Packet*>(msg);
                send(msg, "phys$o");
                scheduleAt(transmissionChannel->getTransmissionFinishTime(), endSuspendTxMsg);
            ...
            {
                if (interfaceEntry)
                    check_and_cast<Packet*>(msg)->addTagIfAbsent<InterfaceInd>()->setInterfaceId(interfaceEntry->getInterfaceId());
                send(msg, "upperLayerOut");
            }
        }
        else if (string(msg->getFullName()).find("Pause") != string::npos)
        {
            if (check_and_cast<Packet*>(msg)->getSchedulingPriority() == 9)
            {
                check_and_cast<Packet*>(msg)->setSchedulingPriority(6); // other mac to encap
                if (transmitState == TX_IDLE_STATE)
                {
                    curTxFrame = check_and_cast<Packet*>(msg);
                    //    sendDelayed(msg, transmissionChannel->getTransmissionFinishTime(), "phys$o");
                    send(msg, "phys$o");
                    scheduleAt(transmissionChannel->getTransmissionFinishTime(), endSuspendTxMsg);
                    changeTransmissionState(TRANSMITTING_STATE);
                    //requestNextFrameFromExtQueue();
                ...
                {
                    NxtPause = check_and_cast<Packet*>(msg);
                }
            }
            else if (check_and_cast<Packet*>(msg)->getSchedulingPriority() == 6)
            {
                if (interfaceEntry)
                    check_and_cast<Packet*>(msg)->addTagIfAbsent<InterfaceInd>()->setInterfaceId(interfaceEntry->getInterfaceId());
                check_and_cast<Packet*>(msg)->setSchedulingPriority(66); // encap to ip
                send(msg, "upperLayerOut");
            }
        }
        else
    ...
    {
        if (string(msg->getFullName()).find("IfRegisterSignal") != string::npos)
        {
            if (interfaceEntry)
                check_and_cast<Packet*>(msg)->addTagIfAbsent<InterfaceInd>()->setInterfaceId(interfaceEntry->getInterfaceId());
            send(msg, "upperLayerOut");
        ...
        {
            check_and_cast<Packet*>(msg)->setSchedulingPriority(99); // encap to ip
            send(msg, "upperLayerOut");
            return;
        }
...
void EtherMacFullDuplex::finish()
{
    ...
    //if (endSuspendTxMsg->isScheduled())
    //    cancelAndDelete(endSuspendTxMsg);
    //delete endSuspendTxMsg;
    /*
    simtime_t t = simTime();
