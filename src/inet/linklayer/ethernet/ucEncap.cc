...
#include "inet/linklayer/ethernet/ucEncap.h"
...
Define_Module(ucEncap);

simsignal_t ucEncap::encapPkSignal = registerSignal("encapPk");
simsignal_t ucEncap::decapPkSignal = registerSignal("decapPk");
simsignal_t ucEncap::pauseSentSignal = registerSignal("pauseSent");

void ucEncap::initialize(int stage)
{
...
void ucEncap::handleMessage(cMessage *msg)
{
...
void ucEncap::processCommandFromHigherLayer(cMessage *msg)
{
    switch (msg->getKind()) {
...
void ucEncap::refreshDisplay() const
{
    char buf[80];
...
void ucEncap::updateRate(simtime_t now)
{
    simtime_t duration = now - intvlStartTime;

    appbitpersec = intvlappBits / duration.dbl();
    cdtbitpersec = intvlcdtBits / duration.dbl();
    ucappbitpersec = intvlucappBits / duration.dbl();

    p_drop = 1- maxcdtrate/(cdtbitpersec + appbitpersec);
    EV<<"updateRate(), new p_drop = "<<p_drop<<endl;

    // restart counters
    intvlStartTime = now;    // FIXME this should be *beginning* of tx of this packet, not end!
    intvlcdtPcks = intvlappBits = intvlucappBits = intvlcdtBits = 0;
    EV<<"updateRate(), the app data rate = "<<appbitpersec<<", the credit rate = "<<cdtbitpersec<<", the uc scheduled data rate = "<<ucappbitpersec<<endl;
}

void ucEncap::processFrameFromMAC(Packet *packet)
{
    // decapsulate and attach control info
    decapsulateMacLlcSnap(packet);

    unsigned long bits = packet->getBitLength();
    if(std::string(packet->getFullName()).find("credit") != std::string::npos)
    {
        intvlcdtBits += bits;
        intvlcdtPcks++;
    }
    if(intvlcdtPcks >= maxcdtInterval)
    {
        EV<<"Receive "<<intvlcdtPcks<<" credits during this interval, update the rate."<<endl;
        updateRate(simTime());
    }

    double temp_ratio = (double)(rand()%99)/(double)99;
    if (temp_ratio < p_drop)
    {
        packet->setKind(1);
    }
...
void ucEncap::processPacketFromHigherLayer(Packet *packet)
{
    ...
    if (std::string(packet->getFullName()).find("UDPBasicAppData") != std::string::npos || std::string(packet->getFullName()).find("UDPBasicAppData") != std::string::npos)
    {
        const auto& ipv4_head = packet->removeAtFront<Ipv4Header>();
        const auto& uc_head = packet->peekAtFront<Ipv4Header>();

        if(uc_head->getDiffServCodePoint() == 11)
        {
            intvlucappBits += bits;
        }
        else
        ...
        packet->insertAtFront(ipv4_head);
    }
    else if (std::string(packet->getFullName()).find("tcpseg") != std::string::npos)
    {
        intvlappBits += bits;
    }
    ...
    ucEncap::addPaddingAndFcs(packet, fcsMode);
...
void ucEncap::addPaddingAndFcs(Packet *packet, FcsMode fcsMode, B requiredMinBytes)
{
    ...
    if (paddingLength > B(0)) {
        ...
        //int jitterbytes = rand()%6;
        int jitterbytes = 0;
        ethPadding->setChunkLength(paddingLength+B(jitterbytes));
        packet->insertAtBack(ethPadding);
    }
    addFcs(packet, fcsMode);
}

void ucEncap::addFcs(Packet *packet, FcsMode fcsMode)
{
...
const Ptr<const EthernetMacHeader> ucEncap::decapsulateMacHeader(Packet *packet)
{
...
const Ptr<const EthernetMacHeader> ucEncap::decapsulateMacLlcSnap(Packet *packet)
{
...
void ucEncap::handleSendPause(cMessage *msg)
{
