...
void EtherEncap::handleMessage(cMessage *msg)
{
    if (msg->arrivedOn("upperLayerIn")) {
        ...
        if (string(check_and_cast<Packet*>(msg)->getFullName()).find("Suspend") != string::npos||string(check_and_cast<Packet*>(msg)->getFullName()).find("IfRegisterSignal") != string::npos)
        {
            send(msg, "lowerLayerOut");
        }
        if (string(check_and_cast<Packet*>(msg)->getFullName()).find("Pause") != string::npos)
        {
            if (check_and_cast<Packet*>(msg)->getSchedulingPriority() == 99)
            {
                check_and_cast<Packet*>(msg)->setSchedulingPriority(9);
                send(msg, "lowerLayerOut");
            }
        }
        else
        ...
        else if (string(check_and_cast<Packet*>(msg)->getFullName()).find("Pause") != string::npos)
        {
            if (check_and_cast<Packet*>(msg)->getSchedulingPriority() == 66)
            {
                check_and_cast<Packet*>(msg)->setSchedulingPriority(99);
                check_and_cast<Packet*>(msg)->addTagIfAbsent<DispatchProtocolReq>()->setProtocol(&Protocol::ipv4);
                send(msg, "upperLayerOut");
            }
            else if (check_and_cast<Packet*>(msg)->getSchedulingPriority() == 99)
            {
                send(msg, "lowerLayerOut");
            }
        }
