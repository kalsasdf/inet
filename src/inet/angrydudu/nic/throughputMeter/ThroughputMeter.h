...
#ifndef __INET_THROUGHPUTMETER_H
#define __INET_THROUGHPUTMETER_H
...
class INET_API ThroughputMeter : public cSimpleModule
{
  protected:
    // config
    ...
    unsigned long intvlappBits;
    unsigned long intvlcdtBits;

    // statistics
    cOutVector bitpersecVector;
    cOutVector pkpersecVector;
    cOutVector goodputVector;
    cOutVector cdtbitpersecVector;

  protected:
    virtual void updateStats(simtime_t now, unsigned long bits, cPacket* pck);
...
#endif // ifndef __INET_THROUGHPUTMETER_H
