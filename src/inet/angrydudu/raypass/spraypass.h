...
#ifndef __INET_SPRAYPASS_H
#define __INET_SPRAYPASS_H
...
class spraypass : public cSimpleModule
{
  protected:
    ...
    double segma = 1; // for ecn based control;
    double times_of_RTT = 2; // for deciding the RTT;
    ...
    static simsignal_t speedflucSignal;
    static simsignal_t updateperiodSignal;
    static simsignal_t updatethetaSignal;

    bool ordered = false;
    cOutVector OoO_Vector;
    cOutVector creditooo_Vector;
    long lastseqno = 0;
    long ooocdts = 0;
  ...
  protected:
    ...
    std::map<short, int64_t> speedboard;
    std::map<short, int64_t> tempboard;
    int validpaths;
    long sumbits_in_period;
    simtime_t last_speedupdate_time;
    double update_bits_th;
    double theta;
    bool pathprobe;
    bool equalspreading;
    double speed_fluctuation;
    ...
    virtual receiver_flowinfo ecnbased_control(receiver_flowinfo tinfo,bool sumrate)
    {
        if(rate_control){
            ...
            else
            {
                ...
                double w = tinfo.omega;
                double nowrate = tinfo.newspeed;
                EV << "feedback_control(), ecn ratio = "<< ecnratio <<", pck_in_rtt = "<<tinfo.pck_in_rtt<<endl;
                if(ecnratio<=targetratio)
                {
                    if(tinfo.previousincrease)
                    {
...
/*
                ...
                {
                    nowrate = nowrate*(1-max_lossratio*segma)*(1+targetratio);
                    if (w/2>wmin)
                        w = w/2;
                    else
                        w = wmin;
...
*/
                ...
                {
                    nowrate = nowrate*(1-ecnratio*segma)*(1+targetratio);
                    if (w/2>wmin)
                        w = w/2;
                    else
                        w = wmin;
...
#endif // ifndef __INET_SPRAYPASS_H
