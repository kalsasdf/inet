...
Define_Module(mpCredits);

simsignal_t mpCredits::queueLengthSignal = registerSignal("queueLength");
simsignal_t mpCredits::ooodegreeSignal = registerSignal("oooDegree");

void mpCredits::initialize()
{
    //statistics
    max_ooo = 0;
...
void mpCredits::handleMessage(cMessage *msg)
{
    if(activate)
    {
        numMapReceived++;
...
void mpCredits::handleSelfMessage(cMessage *pck)
{
...
void mpCredits::refreshDisplay() const
{
    char buf[100];
...
void mpCredits::processUpperpck(Packet *pck)
{
...
void mpCredits::processLowerpck(Packet *pck)
{
...
Packet *mpCredits::findData(L3Address addr)
{
...
Packet *mpCredits::extractData(L3Address addr)
{
    auto it = data_Map.begin();
    for(; it != data_Map.end(); ++ it)
    {
...
Packet *mpCredits::insertData(L3Address addr,Packet *pck)
{
...
Packet *mpCredits::extractCredit(L3Address addr)
{
...
Packet *mpCredits::findCredit(L3Address addr)
{
    auto it = credit_Map.begin();
    for(; it != credit_Map.end(); ++ it)
    {
...
Packet *mpCredits::insertCredit(L3Address addr,Packet *pck)
{
if (useECN)
{
...
Packet *mpCredits::newflowinfo(Packet *pck)
{
    auto l3addr = pck->addTagIfAbsent<L3AddressReq>();
...
int mpCredits::deleteFlowCredits(L3Address addr)
{
    int sum_deleted;
...
void mpCredits::sendDown(Packet *pck)
{
    EV << "00000000 Oh send down, data map size = " << data_Map.size() << " ! 00000000"<<endl;
    send(pck, outGate);
}

void mpCredits::sendUp(Packet *pck)
{
    EV<<"mpcredits, oh sendup!"<<endl;
    send(pck, upGate);
    //EV<<"mpcredits, oh sendup!"<<endl;
    //send(pck, upGate);
}

void mpCredits::send_credreq(L3Address destaddr)
{
...
void mpCredits::send_credit(L3Address destaddr,unsigned int pathid, int seq)
{
...
void mpCredits::send_stop(L3Address addr)
{
...
void mpCredits::receive_credreq(Packet *pck)
{
...
void mpCredits::receive_credit(Packet *pck)
{
    auto l3addr = pck->addTagIfAbsent<L3AddressInd>();
    bool stop = false;
    L3Address stopaddr;
    ...
    if (data == nullptr) {
        ...
        data_head->setDiffServCodePoint(11); // determine the data packets is scheduled by mpCredits
        data_head->setExplicitCongestionNotification(ecnInd->getExplicitCongestionNotification());
...
void mpCredits::receive_stopcred(Packet *pck)
{
...
void mpCredits::receive_data(Packet *pck)
{
...
void mpCredits::orderpackets(L3Address addr)
{
...
void mpCredits::self_send_credit()
{
...
void mpCredits::self_stop_credit()
{
...
void mpCredits::schedule_next_credit(simtime_t delta_t)
{
...
simtime_t mpCredits::get_singleRTT(Packet *pck)
{
...
void mpCredits::multipath_control(L3Address addr, double speed, double oldspeed, double max_sumspeed)
{
    if (rate_control)
    {
