...
simsignal_t UC::queueLengthSignal = registerSignal("queueLength");
simsignal_t UC::ooodegreeSignal = registerSignal("oooDegree");
...
void UC::handleMessage(cMessage *msg)
{
    if(activate)
    {
        numMapReceived++;
...
void UC::handleSelfMessage(cMessage *pck)
{
...
void UC::processUpperpck(Packet *pck)
{
...
void UC::processLowerpck(Packet *pck)
{
...
Packet *UC::findData(L3Address addr)
{
...
Packet *UC::extractData(L3Address addr)
{
    auto it = data_Map.begin();
    for(; it != data_Map.end(); ++ it)
    {
...
Packet *UC::insertData(L3Address addr,Packet *pck)
{
...
Packet *UC::extractCredit(L3Address addr)
{
...
Packet *UC::findCredit(L3Address addr)
{
    auto it = credit_Map.begin();
    for(; it != credit_Map.end(); ++ it)
    {
...
Packet *UC::insertCredit(L3Address addr,Packet *pck)
{
if (useECN)
{
...
Packet *UC::newflowinfo(Packet *pck)
{
    auto l3addr = pck->addTagIfAbsent<L3AddressReq>();
...
int UC::deleteFlowCredits(L3Address addr)
{
    int sum_deleted;
...
void UC::sendDown(Packet *pck)
{
    EV << "00000000 Oh send down, data map size = " << data_Map.size() << " ! 00000000"<<endl;
    send(pck, outGate);
}

void UC::sendUp(Packet *pck)
{
    EV<<"UC, oh sendup!"<<endl;
    send(pck,upGate);

}

void UC::send_credreq(L3Address destaddr)
{
...
void UC::send_credit(L3Address destaddr,unsigned int pathid, int seq)
{
...
void UC::send_stop(L3Address addr)
{
...
void UC::receive_credreq(Packet *pck)
{
...
void UC::receive_credit(Packet *pck)
{
    auto l3addr = pck->addTagIfAbsent<L3AddressInd>();
    bool stop = false;
    L3Address stopaddr;
    ...
    if (data == nullptr) {
        ...
        data_head->setDiffServCodePoint(11); // determine the data packets is scheduled by UC
        data_head->setExplicitCongestionNotification(ecnInd->getExplicitCongestionNotification());
...
void UC::receive_stopcred(Packet *pck)
{
...
void UC::receive_data(Packet *pck)
{
...
bool UC::orderpackets(L3Address addr)
{
    ...
    for(; it != receiver_orderMap.end(); ++ it)
    {
        ...
        if (addr == it->first && tdata_head->getIdentification() == receiver_seqCache.find(addr)->second + 1)
        {
            ...
            globaltimes++;
            EV<<"UC, oh sendup! time = "<<SimTime()<<endl;
...
void UC::schedule_next_credit(simtime_t delta_t)
{
...
void UC::multipath_control(L3Address addr, double speed, double oldspeed, double max_sumspeed)
{
    if (rate_control)
    {
